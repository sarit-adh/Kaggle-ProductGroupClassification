...
#parameter_grid = {'n_neighbors':[1, 2,5,10]}

#kNeighborsCVClassifier = model_selection.GridSearchCV(svmClassifier, parameter_grid)
#kNeighborsCVClassifier.fit(X_train, y_train)



#prediction in test set

prediction =  kNeighborsClassifier.predict_proba(X_test)
prediction_with_id = np.hstack((IDs_test,prediction))
prediction_with_id[:,0] = prediction_with_id[:,0].astype(int)
   
np.savetxt('submission_KNN.csv', prediction_with_id,delimiter=',',fmt=','.join(['%i'] + ['%f']*9))

with open('submission_KNN.csv', 'r+') as f:
        content = f.read()
        f.seek(0, 0)
