...
calibrated_randomForestClassifier = CalibratedClassifierCV(randomForestClassifier,method='isotonic',cv=5)
calibrated_randomForestClassifier.fit(X_train,y_train) 
print('Random Forest Classifier LogLoss {score}').format(score=log_loss(y_val,calibrated_randomForestClassifier.predict_proba(X_val)))
classifiers.append(calibrated_randomForestClassifier)

#randomForestClassifier2 = RandomForestClassifier(n_estimators=1000, random_state=2, n_jobs=-1)
#calibrated_randomForestClassifier2 = CalibratedClassifierCV(randomForestClassifier,method='isotonic',cv=5)
#calibrated_randomForestClassifier2.fit(X_train,y_train)
#print('Random Forest Classifier 2 LogLoss {score}').format(score=log_loss(y_val,calibrated_randomForestClassifier2.predict_proba(X_val)))
#classifiers.append(calibrated_randomForestClassifier2)
#
...
kNeighborsClassifier = KNeighborsClassifier(n_neighbors=5) #default 5
calibrated_kNeighborsClassifier = CalibratedClassifierCV(kNeighborsClassifier,method='isotonic',cv=5)
calibrated_kNeighborsClassifier.fit(X_train,y_train)
print('K Neighbors Classifier LogLoss {score}').format(score=log_loss(y_val,calibrated_kNeighborsClassifier.predict_proba(X_val)))
classifiers.append(calibrated_kNeighborsClassifier)
